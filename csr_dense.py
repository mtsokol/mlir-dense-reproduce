...
with Context():
    ...
    func.func @add(%st_0 : tensor<3x4xf64, #CSR>, %st_1 : tensor<3x4xf64, #Dense>) attributes { llvm.emit_c_interface } {
        %out_st = tensor.empty() : tensor<3x4xf64, #Dense>
        %res = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%st_0, %st_1 : tensor<3x4xf64, #CSR>, tensor<3x4xf64, #Dense>) outs(%out_st : tensor<3x4xf64, #Dense>) {
    ...
    data = np.array([1, 2, 1, 1, 3], dtype=np.float64)
    pos_memref = rt.get_ranked_memref_descriptor(pos)
    idx_memref = rt.get_ranked_memref_descriptor(idx)
    data_memref = rt.get_ranked_memref_descriptor(data)
    ...
    ret = ee_add.invoke(
        "add",
        # CSR input
        ctypes.pointer(ctypes.pointer(pos_memref)),
        ctypes.pointer(ctypes.pointer(idx_memref)),
        ctypes.pointer(ctypes.pointer(data_memref)),
        # Dense input
